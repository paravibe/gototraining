...
class GoToTrainingRequest {
  ...
  public function __construct($requestType, $endpoint, $accessToken, $baseUrl, $apiPrefix) {
    ...
    if (!$this->accessToken) {
      throw new GoToTrainingException(GoToTrainingConstants::ERROR_ACCESS_TOKEN);
    }
  ...
  public function execute($client = NULL) {
    ...
    try {
      ...
      $error = new GoToTrainingResponse($result);

      throw new GoToTrainingException($error->getBody(), $error->getHttpStatusCode());
    }

    return new GoToTrainingResponse($result);
  }
   ...
   *
   * @return \LogMeIn\GoToTraining\Http\GoToTrainingRequest
   *   GoToTrainingRequest object.
   */
